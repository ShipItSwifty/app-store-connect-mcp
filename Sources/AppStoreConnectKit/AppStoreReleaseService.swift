...
public struct AppStoreReleaseService: Sendable {
    ...
    private let logger = Logger.forType(subsystem: "AppStoreConnectKit", AppStoreReleaseService.self)
    ...
    /// - Throws: `ASCError.apiError` if the app or API calls fail.
    ...
    ///   - resolveVersionString: Called only when a new App Store version must be created;
    ///     returns the marketing version string (e.g. `"1.4.0"`).
    /// - Returns: A `MetadataSyncResult` summarising how many locales were processed.
    /// - Throws: `ASCError.apiError` if the app or API calls fail.
    ...
    private func resolveAppInfoID(appID: String) async throws -> String {
        ...
        guard let appInfo = response.data.first else {
            throw ASCError.apiError(statusCode: 404, body: "App info for app '\(appID)' not found in App Store Connect")
        }

        return appInfo.id
    }

    private func resolveOrCreateAppStoreVersion(
        appID: String,
        resolveVersionString: @Sendable () async throws -> String
    ) async throws -> AppStoreVersionResource {
        if let existing = try await resolveLatestAppStoreVersion(appID: appID) {
            return existing
        }

        let versionString = try await resolveVersionString()
