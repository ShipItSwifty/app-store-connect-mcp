...
public actor JWTGenerator {
    ...
    private let logger = Logger.forType(subsystem: "AppStoreConnectKit", JWTGenerator.self)
    ...
    /// - Throws: ``ASCError/jwtGenerationFailed(underlying:)`` if signing fails.
    public func generateToken(
        ...
        do {
            ...
            throw ASCError.jwtGenerationFailed(underlying: error)
        }
    }
    ...
    public func cachedOrNewToken() async throws -> String {
        ...
        let task = Task { [weak self] () throws -> String in
            guard let self else { throw ASCError.jwtGenerationFailed(underlying: CancellationError()) }
