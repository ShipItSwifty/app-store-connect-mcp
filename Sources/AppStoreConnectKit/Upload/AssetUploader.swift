...
public struct AssetUploader: Sendable {
    ...
    private let logger = Logger.forType(subsystem: "AppStoreConnectKit", AssetUploader.self)
    ...
    /// - Throws: ``ASCError/uploadFailed(asset:reason:)`` on unrecoverable failure after retries.
    public func upload(
        fileURL: URL,
    ...
    private func uploadPart(
        ...
        guard let url = URL(string: operation.url) else {
            throw ASCError.uploadFailed(asset: fileName, reason: "Invalid upload URL: \(operation.url)")
        }
        ...
        guard let httpResponse = response as? HTTPURLResponse else {
            throw ASCError.uploadFailed(asset: fileName, reason: "Invalid response type")
        }
    ...
    private func readPart(from fileURL: URL, offset: Int, length: Int) throws -> Data {
        guard let fileHandle = FileHandle(forReadingAtPath: fileURL.path) else {
            throw ASCError.uploadFailed(asset: fileURL.lastPathComponent, reason: "Cannot open file for reading")
        }
        ...
        guard let data = try fileHandle.read(upToCount: length) as Data? ?? fileHandle.readData(ofLength: length) as Data? else {
            throw ASCError.uploadFailed(asset: fileURL.lastPathComponent, reason: "Failed to read part data")
        }
        return data
    }
    ...
    private func fileSize(at url: URL) throws -> Int {
        ...
        guard let size = attrs[.size] as? Int else {
            throw ASCError.uploadFailed(asset: url.lastPathComponent, reason: "Cannot determine file size")
        }
        return size
    }
    ...
    private func computeMD5(at url: URL) throws -> String {
        guard let inputStream = InputStream(url: url) else {
            throw ASCError.uploadFailed(asset: url.lastPathComponent, reason: "Cannot open file for MD5 computation")
        }
        ...
        while inputStream.hasBytesAvailable {
            ...
            if bytesRead < 0 {
                throw ASCError.uploadFailed(asset: url.lastPathComponent, reason: "Error reading file for MD5")
            }
